...
struct board_state_variables                                                                   // copiado
{
...
char *connection_info_filename = "connection_info.dat";
uint8_t first = 0;
/*******************************************************************************
 * Code
 ******************************************************************************/
...
static int CGI_HandlePost(HTTPSRV_CGI_REQ_STRUCT *param)
{
    ...
    else
    {
        ...
        vTaskResume(g_BoardState.prvsngTask);
    }
...
static void provisioning_task(void *arg)
{
    ...
    if(!first){
    	while (opt <= 0 || opt >= 3) {
            printf("Quieres reiniciar los datos de la red?\n");
            printf("Presiona 1 para SI y 2 para NO.\n");
            ...
            if (opt <= 0 || opt >= 3) {
                printf("Opción inválida. Debes ingresar 1 o 2.\n");
            }
        }
    	first = 1;

    	if(opt == 1)
    	    	connection_info_filename = "connection_info2.dat";
    }
...
int main(void)
{
    ...
    if (xTaskCreate(provisioning_task, "provisioning_task", 2048, NULL, configMAX_PRIORITIES - 4, &g_BoardState.prvsngTask) != pdPASS)
    {
